...
////// 新加部分 start //////
  ...
  if(pa == 0)
    panic("kalloc");
  uint64 va = KSTACK((int)0); // 将内核栈映射到固定的逻辑地址上 ,KSTACK(p)中为guard page分配了虚拟内存
...
growproc(int n)
{
  ...
  if(n > 0){
    uint64 newsz;
    if((newsz = uvmalloc(p->pagetable, sz, sz + n)) == 0) {   //uvmalloc返回newsz的值
      return -1;
    }

    //内核页表中的映射同步扩大
    if(kvmcopymappings(p->pagetable, p->kernelpgtbl, sz, n) != 0){  //kvmcopymappings失败之后，只取消了映射，没有释放物理内存
      uvmdealloc(p->pagetable, newsz, sz);        //映射失败，释放掉物理内存
      return -1;
    }
    sz = newsz;
  } else if(n < 0){
    uvmdealloc(p->pagetable, sz, sz + n);  //n < 0, 用户页表缩小
    //内核页表中的映射同步缩小
...
fork(void)
{
  ...
  //将新进程的用户页表拷贝一份到内核页表中
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0 || kvmcopymappings(np->pagetable, np->kernelpgtbl, 0, p->sz) < 0){  //sz 进程内存的大小
    freeproc(np);
    release(&np->lock);
    return -1;
  }
